...
**Status:** `partial / endpoint-paired where verified`  
**Reviewed:** 2026-07-12
...
| `HMCD(StartPlayersRoleSelection)` | S -> selected C and C -> S acknowledgement | S writes role string; C acknowledgement has no payload in traced path | server checks sender membership in choosing list; feature hard-disabled upstream | matched branch but unreachable |
| `HMCD(EndPlayersRoleSelection)` | S -> C | none | reader not fully traced | partial |
| `HMCD(SetSubRole)` | mixed/unresolved | client reader expects string; sender/validator unresolved | trust and allowed values unresolved | partial |
| `HMCDPoliceRole` | unresolved | unresolved | endpoints unresolved | partial |
| `hmcd_announce_traitor_lose` | S -> C | entity traitor; bool alive | entity validation/client behavior incomplete | partial |
| `HMCD_TraitorDeathState` | S -> main traitor C | string appearance name; bool alive | server recipient-filtered | matched schema |
...
| `HMCD_UpdateTraitorAssistants` | S -> main traitor C | uint8 count; repeated color, string name, string SteamID | client receiver not located | partial |
...
- Resolve Defense auxiliary files, Counter-Strike objective entities, Homicide/Fear missing receivers, Pathowogen derma files and legacy admin clients.
